...
modelfile = os.path.join(script_dir, "voting_model.pkl")

# Try to load the model with explicit error handling
try:
    voting_model = pickle.load(open(modelfile, "rb"))
except FileNotFoundError:
    st.error(f"Model file not found at: {modelfile}")
    st.info("Please make sure the model file 'voting_model.pkl' is in the same directory as this script.")
...
with st.sidebar:
    # Use OS-independent path handling for image file
    image_path = os.path.join(script_dir, "assets", "supply_chain_optimisation.jpg")
    try:
        img = Image.open(image_path)
    except FileNotFoundError:
        st.warning(f"Image not found at: {image_path}")
        # Continue without showing the image if it's not found
        img = None
